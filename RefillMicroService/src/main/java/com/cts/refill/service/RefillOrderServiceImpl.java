...
public class RefillOrderServiceImpl implements IRefillOrder {
	...
	public List<RefillOrder> getStatus(long subID, String token)
			throws SubscriptionIDNotFoundException, com.cts.refill.exception.InvalidTokenException {
	...
	@Override
	public List<RefillOrderLine> getRefillDuesAsOfDate(String memberId, int date, String token)
	...
	@Override
	public RefillOrder requestAdhocRefill(long policyId, long subID, String memberID, String location, int quantity,
			String token) throws ParseException, DrugQuantityNotAvailable, InvalidTokenException, FeignException {
		log.info("inside requestAdhocRefill");

		if (authFeign.getValidity(token).isValid()) {
			// ResponseEntity<String> drugrName =
			// subClient.getDrugNameBySubscriptionId(token,memberID);
